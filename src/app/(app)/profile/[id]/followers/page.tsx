    ...
    async function load() {
      try {
        ...
        if (res.status === 403) {
          ...
          if (data.private) {
            setIsPrivate(true);
            // Still try to get the profile name
            try {
    ...
    <main className="px-4 pt-4 pb-20">
      ...
      {!loading && isPrivate && (
        <div className="text-center py-16">
          ...
          <h2 className="text-lg font-bold text-gray-900 mb-2">Followers List is Private</h2>
          <p className="text-sm text-gray-500 max-w-xs mx-auto">
            {profileName || "This user"} has chosen to keep their followers list private.
          </p>
        </div>
      )}
      ...
      {!loading && !isPrivate && users.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-sm text-gray-500">When people follow this account, they&apos;ll show up here.</p>
        </div>
      )}
